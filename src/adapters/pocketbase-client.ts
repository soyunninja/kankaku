...
export class PocketBaseClient {
  ...
  /** Perform `request` with the current token (authenticating first if there is none). */
  private async authenticate(): Promise<void> {
    const response = await this.rawFetch("POST", AUTH_PATH, { identity: this.email, password: this.password }, undefined);
    if (!response.ok) {
  ...
  private async rawFetch(method: string, path: string, body: unknown, token: string | undefined): Promise<Response> {
  ...
  async request<T = unknown>(method: string, path: string, body?: unknown): Promise<T> {
    ...
    let response = await this.rawFetch(method, path, body, this.token);

    if (response.status === 401) {
      this.token = undefined;
      await this.authenticate();
      response = await this.rawFetch(method, path, body, this.token);
    }
   ...
   * verbatim as PocketBase filter/sort expressions.
   */
  async list<T extends PocketBaseRecord = PocketBaseRecord>(
    ...
    for (;;) {
      const result = await this.request<PocketBaseListResult<T>>("GET", listPath(collection, page, perPage, opts.filter, opts.sort));
